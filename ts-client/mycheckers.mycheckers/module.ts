...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgPlayMove({ value }: msgPlayMoveParams): EncodeObject {
			try {
				return { typeUrl: "/mycheckers.mycheckers.MsgPlayMove", value: MsgPlayMove.fromPartial( value ) }  
			} catch (e: any) {
		...
		msgCreateGame({ value }: msgCreateGameParams): EncodeObject {
			try {
				return { typeUrl: "/mycheckers.mycheckers.MsgCreateGame", value: MsgCreateGame.fromPartial( value ) }  
			} catch (e: any) {
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			MycheckersMycheckers: new SDKModule(test)
		},
		registry: msgTypes
  }
}
